...
fn uuid8_from_binary(inputs: &[Series]) -> PolarsResult<Series> {
    ...
    let out: StringChunked = values.apply_into_string_amortized(|bytes: &[u8], output: &mut String| {
...
fn uuid8_by_hashing_string(inputs: &[Series]) -> PolarsResult<Series> {
    ...
    let out: StringChunked = values.apply_into_string_amortized(|value: &str, output: &mut String| {
